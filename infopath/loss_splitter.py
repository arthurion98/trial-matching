...
class MultiTaskSplitter(nn.Module):
    def __init__(self, num_copies, symmetric=True, random_order=True):
        ...
        self.random_order = random_order if not symmetric else None
...
class GradientNormalizingFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, m: torch.Tensor, t: int, beta: float, epsilon: float):
    ...
    def backward(ctx, grads):
        ...
        epsilon = torch.tensor(epsilon, device=m.device)
        if len(m.shape) == 1 and m.shape[0] == 1:
            ...
            assert (
                grad_squared.shape == m.shape
            ), "got gradient with shape: {} variance accumulated: {} ".format(
                grad_squared.shape, m.shape
            )

        if grad_squared.sum() == 0.0:  # strict zero no update of the momentum
            return torch.zeros_like(grads), None, None, None, None
...
class GradientNormalizer(nn.Module):
    def __init__(
        self, feature_shape=[1], beta=0.999, epsilon=1e-16, dtype=torch.float32
    ):
    ...
    def forward(self, x):
        x = self.function.apply(
            x, self._grad_var.data, self._t.data, self.beta, self.epsilon
        )
        return x
...
class NormalizedMultiTaskSplitter(nn.Module):
    def __init__(
        ...
        self.splitter = MultiTaskSplitter(
            num_copies=num_copies, random_order=random_order, symmetric=symmetric
        )
        make_normalizer = lambda: GradientNormalizer(
            feature_shape, beta, epsilon, dtype
        )
...
if __name__ == "__main__":
    ...
    plt.scatter(
        x_list[:, 0], x_list[:, 1], c=np.linspace(0, 1, x_list.shape[0]), marker="."
    )
